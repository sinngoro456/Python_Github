...
def tikhonov_deblurring(noisy_blurred_image, kernel, lambda_reg=10000):
    ...
    identity = np.ones(image_size)  # 恒等演算子
    F_deblurred_tikhonov = (F_blurred * np.conj(F_kernel)) / (
        np.abs(F_kernel) ** 2 + lambda_reg * np.abs(identity) ** 2
    )
...
# ウィナー逆フィルターを適用
def wiener_deblurring(noisy_blurred_image, kernel, K=1):
    image_size = noisy_blurred_image.shape  # 画像のサイズを取得
    kernel_padded = np.zeros(image_size)  # ゼロパディングカーネル
    kh, kw = kernel.shape  # カーネルの高さ・幅
    kernel_padded[:kh, :kw] = kernel
    ...
    F_wiener = (np.conj(F_kernel) / np.abs(F_kernel) ** 2) * (
        (np.abs(F_kernel) ** 2) / (np.abs(F_kernel) ** 2 + K)
    )
    F_deblurred_wiener = F_blurred * F_wiener  # フィルタ適用

    return np.real(ifft2(F_deblurred_wiener))
...
def process_image():
    ...
    deblurred_wiener = wiener_deblurring(noisy_blurred_image, kernel)
    ...
    axes[4].imshow(deblurred_wiener, cmap="gray")
    axes[4].set_title("Deblurred Image (Wiener)")
    plt.show()
